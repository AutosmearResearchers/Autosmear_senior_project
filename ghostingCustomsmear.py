...
def get_custom_smear_frame(custom_frame=1):
    ...
    '''
    custom_smear_frame = [custom_frame]
    
    return custom_smear_frame
...
def get_values(
    ...
    Args:
        custom_frame(int): the frame in which the smear must be generated
    '''
...
############################################################################
    ...
    history_dict = "{frame}||{ghost_grp}".format(frame=custom_frame, ghost_grp=group_name)
    attr_naming = "ghosting_{keyframe}_{group}".format(group= group_name,keyframe=custom_frame)
